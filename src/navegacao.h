...
//   moverCursor(dir)      -> setas: move o cursor (grade no menu, lista no
//   resto)
// ===========================================================================
...
struct EstadoTela {
  ...
  char
      categoria[16]; // contexto: categoria do cardapio (LISTA_PRODUTOS/DETALHE)
...
void renderizarTelaAtual() {
  ...
  case CARREGANDO:
    ...
    if (clienteCarregado && estado.indice == ESPERANDO_CLIENTE) {
      ...
      estado.indice = 0;
      renderizarTelaAtual();
    } else if (meusPedidosProntos && totalContaProntas &&
               estado.indice == ESPERANDO_MEUS_PEDIDOS) {
      estado.tipo = MEUS_PEDIDOS;
      estado.indice = 0;
      renderizarTelaAtual();
      ...
      estado.indice = ESPERANDO_PIX;
      renderizarTelaAtual();
    } else if (pixPronto && estado.indice == ESPERANDO_PIX) {
      estado.tipo = PAGAR_CONTA;
      estado.indice = 0;
      renderizarTelaAtual();
    }
    break;
  ...
  case LISTA_PRODUTOS: {
    ...
    if (strcmp(estado.categoria, "Drinks") == 0)
      titulo = "Drinks";
    else if (strcmp(estado.categoria, "Petiscos") == 0)
      titulo = "Petiscos";
    else if (strcmp(estado.categoria, "Nao Alcoolico") == 0)
      titulo = "Nao Alcoolico";
  ...
  case CONTROLE_CERVEJA: {
    ...
    desenharControleCerveja(quantidadeMl, temperaturaC, status.c_str(),
                            nivelBateria);
    break;
  }
  ...
  case PAGAR_CONTA:
    if (pagamentoConfirmado) {
      ...
      estado.tipo = PIX_CONFIRMADO;
      instanteConfirmado = millis();

      renderizarTelaAtual();
      break;
    }

    desenharPagarConta(totalContaAtual, false);
    break;

  case PIX_CONFIRMADO:
    desenharPagarConta(totalContaAtual, true);
    break;
...
void selecionar() {
  ...
  case MENU:
    if (estado.indice == 0) {
      ...
      estado.indice = ESPERANDO_PARTIDAS;
      renderizarTelaAtual();
      ...
      empilhar();
      estado.tipo = CARREGANDO;
      estado.indice = ESPERANDO_APOSTAS;
      renderizarTelaAtual();
    }
    break;
  ...
  case CARDAPIO:
    if (estado.indice == 0) {
      ...
      empilhar();
      estado.tipo = CARREGANDO;
      estado.indice = ESPERANDO_PRODUTOS;
      renderizarTelaAtual();
    }
    break;
  ...
  case CONFIRMACAO: {
    ...
    registrarAposta(jogoId,
                    estado.palpite); // MOCK: grava no banco no id do usuario
  ...
  case MENU_PEDIDOS:
    if (estado.indice == 0) {
      ...
      empilhar();
      estado.tipo = CARREGANDO;
      estado.indice = ESPERANDO_PAGAR_CONTA;
      renderizarTelaAtual();
    }
    break;
...
void moverCursor(Direcao dir) {
  ...
  case MENU: { // grade 2x2: 0=cardapio 1=apostar / 2=pedidos 3=minhas apostas
    if (dir == CIMA && estado.indice >= 2)
      estado.indice -= 2;
    else if (dir == BAIXO && estado.indice <= 1)
      estado.indice += 2;
    else if (dir == ESQUERDA && (estado.indice % 2) == 1)
      estado.indice -= 1;
    else if (dir == DIREITA && (estado.indice % 2) == 0)
      estado.indice += 1;
    else
      return;
    renderizarTelaAtual();
  ...
  case CARDAPIO: { // lista de 4
    if (dir == CIMA && estado.indice > 0)
      estado.indice--;
    else if (dir == BAIXO && estado.indice < 3)
      estado.indice++;
    else
      return;
    renderizarTelaAtual();
  ...
  case LISTA_PRODUTOS: { // lista de tamanho dinamico
    ...
    if (dir == CIMA && estado.indice > 0)
      estado.indice--;
    else if (dir == BAIXO && estado.indice < n - 1)
      estado.indice++;
    else
      return;
    renderizarTelaAtual();
  ...
  case JOGO: { // 3 palpites
    if (dir == CIMA && estado.indice > 0)
      estado.indice--;
    else if (dir == BAIXO && estado.indice < 2)
      estado.indice++;
    else
      return;
    renderizarTelaAtual();
...
void botaoMidPressionado(GFButton &b) {
  registrarAtividade();
  selecionar();
}
void botaoUpPressionado(GFButton &b) {
  registrarAtividade();
  moverCursor(CIMA);
}
void botaoDownPressionado(GFButton &b) {
  registrarAtividade();
  moverCursor(BAIXO);
}
